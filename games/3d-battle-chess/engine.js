...
export class ChessGame{
 ...
 reset(){this.board=Array.from({length:8},()=>Array(8).fill(null));for(let x=0;x<8;x++){this.board[0][x]={c:'b',t:BACK[x]};this.board[1][x]={c:'b',t:'p'};this.board[6][x]={c:'w',t:'p'};this.board[7][x]={c:'w',t:BACK[x]}}this.turn='w';this.castling={w:{k:true,q:true},b:{k:true,q:true}};this.ep=null;this.halfmove=0;this.fullmove=1;this.history=[];this.moves=[];this.positions=new Map();this.recordPosition()}
 snapshot(){return{board:copy(this.board),turn:this.turn,castling:structuredClone(this.castling),ep:this.ep?{...this.ep}:null,halfmove:this.halfmove,fullmove:this.fullmove,moves:this.moves.map(m=>({...m})),positions:new Map(this.positions)}}
 restore(s){this.board=copy(s.board);this.turn=s.turn;this.castling=structuredClone(s.castling);this.ep=s.ep?{...s.ep}:null;this.halfmove=s.halfmove;this.fullmove=s.fullmove;this.moves=s.moves.map(m=>({...m}));this.positions=new Map(s.positions)}
