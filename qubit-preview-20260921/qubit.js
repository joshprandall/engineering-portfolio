export function degreesToRadians(degrees) {
  if (!Number.isFinite(degrees) || degrees < 0 || degrees > 180) {
    throw new RangeError("theta must be a finite number from 0 through 180 degrees");
  }
  return degrees * Math.PI / 180;
}

export function stateFromAngle(thetaDegrees) {
  const theta = degreesToRadians(thetaDegrees);
...
export function sampleMeasurements(p0, shots, rng = Math.random) {
  if (!Number.isFinite(p0) || p0 < 0 || p0 > 1) {
    throw new RangeError("p0 must be between 0 and 1");
  }
