(() => {
  ...
  const subjectParam=new URL(location.href).searchParams.get('domain');
  const activeSubject=D?.domains.some(d=>d.id===subjectParam)?subjectParam:null;
  if (!D) return;
  ...
  function renderDomains(){
    ...
    $$('.domain-card').forEach(card=>{
      const go=()=>{ location.href=`learn-${encodeURIComponent(card.dataset.domain)}.html`; };
  ...
  function startSprint(){const pool=D.lessons.filter(l=>l.quiz&&(!activeSubject||l.domain===activeSubject));const shuffled=[...pool].sort(()=>Math.random()-.5).slice(0,5);sprintState={items:shuffled,index:0,score:0,answered:false};renderSprint()}
