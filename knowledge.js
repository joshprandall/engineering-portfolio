(() => {
  ...
  function audienceLabel(){return learnerMode==='starter'?'Starter / young learner':learnerMode==='technical'?'Technical depth':'General learner'}
  function modeSwitcherHTML(){return `<div class="learning-mode-bar"><div><span class="eyebrow">LEARNING MODE</span><strong>${esc(audienceLabel())}</strong><small>Same verified claim, different scaffolding.</small></div><label>Depth <select id="learner-mode" aria-label="Learning mode"><option value="starter" ${learnerMode==='starter'?'selected':''}>Starter / young learner</option><option value="general" ${learnerMode==='general'?'selected':''}>General learner</option><option value="technical" ${learnerMode==='technical'?'selected':''}>Technical depth</option></select></label></div>`}
