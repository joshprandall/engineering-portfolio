...
4. Copy the root HTML, CSS, and JS site files, `deep-learning/`, `geometric-lab/`, `games/battle-chess/`, `labs/`, `qubit-preview-20260921/`, and needed static assets into the existing `public_html`, retaining the destination's existing `games/evil-wizard/` browser export, fusion video, and any other local assets. Merge directories; never delete a destination directory that contains assets missing from the source repository. Retain `resume.html` and any newer host-owned files unless specifically updated.
